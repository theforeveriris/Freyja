...
from src.database import get_analysis_by_id, get_or_create_user
...
EXPERT_CATEGORIES = {
    ...
    'prosody': {'name': '🗣️ 发声模式分析', 'description': 'VOT / 开商 / 气息声 / 起音'},
    'gender': {'name': '🧭 性别特征深度分析', 'description': '五维雷达图 / 详细解读'},
    'charts_all': {'name': '🖼️ 全部可视化图表', 'description': '一次性发送所有12张图'}
}
...
async def expert(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        analysis = db.query(db.model.Analysis).filter(db.model.Analysis.user_id == user.user_id).order_by(db.model.Analysis.id.desc()).first()
        if not analysis:
...
"""
        ...
        if len(f0_values) > 0:
            ...
            visualizer.plot_pitch_contour(f0_values, analysis.sample_rate, pitch_contour_path)
            ...
            visualizer.plot_pitch_histogram(f0_values, pitch_hist_path, male_range, female_range)
            
            await query.message.reply_text(report, parse_mode='MarkdownV2')
...
"""
        ...
        if len(f1_values) > 0:
            ...
            visualizer.plot_formants(f1_values, f2_values, f3_values, analysis.sample_rate, formant_path)
            
            scatter_path = os.path.join(charts_dir, 'formant_scatter.png')
            visualizer.plot_formant_scatter(f1_values, f2_values, scatter_path)
...
"""
        ...
        if len(energy_values) > 0:
            ...
            visualizer.plot_energy_envelope(energy_values, analysis.sample_rate, energy_path)
            await query.message.reply_text(report, parse_mode='MarkdownV2')
...
"""
        ...
        visualizer.plot_gender_radar(params, radar_path)
...
async def send_all_charts(query, analysis, params, settings):
    ...
    await query.message.reply_text("🖼️ 正在生成所有图表...请稍候")
    
    try:
        import librosa
        audio_data, sr = librosa.load(analysis.audio_path, sr=None)
        
        waveform_path = os.path.join(charts_dir, 'waveform.png')
        visualizer.plot_waveform(audio_data, sr, waveform_path)
        ...
        visualizer.plot_spectrogram(audio_data, sr, spectrogram_path)
        await query.message.reply_photo(open(spectrogram_path, 'rb'), caption='语谱图')
        
        f0_values = np.array(params.get('f0_values', []))
        if len(f0_values) > 0:
            pitch_contour_path = os.path.join(charts_dir, 'pitch_contour.png')
            visualizer.plot_pitch_contour(f0_values, sr, pitch_contour_path)
            await query.message.reply_photo(open(pitch_contour_path, 'rb'), caption='基频轮廓图')
        ...
        if len(f1_values) > 0:
            formant_path = os.path.join(charts_dir, 'formants.png')
            visualizer.plot_formants(f1_values, f2_values, f3_values, sr, formant_path)
            await query.message.reply_photo(open(formant_path, 'rb'), caption='共振峰轨迹图')
        ...
        if len(energy_values) > 0:
            energy_path = os.path.join(charts_dir, 'energy_envelope.png')
            visualizer.plot_energy_envelope(energy_values, sr, energy_path)
            await query.message.reply_photo(open(energy_path, 'rb'), caption='能量包络图')
        ...
        if len(centroid_values) > 0:
            ...
            visualizer.plot_spectral_centroid(centroid_values, sr, centroid_path)
        ...
        visualizer.plot_mel_spectrogram(audio_data, sr, mel_spec_path)
        ...
        if len(mfcc_values) > 0:
            ...
            visualizer.plot_mfcc(mfcc_values, mfcc_path)
