...
from src.database import get_user_analyses, get_or_create_user, get_analysis_by_id
...
async def compare(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 2:
        await update.message.reply_text("请指定两个要对比的记录 ID，如 `/compare 1 2`")
        return
    ...
    analysis1 = get_analysis_by_id(db, id1)
    analysis2 = get_analysis_by_id(db, id2)
    
    if not analysis1 or not analysis2:
        await update.message.reply_text("未找到指定记录")
        return
    
    if analysis1.user_id != update.effective_user.id or analysis2.user_id != update.effective_user.id:
        await update.message.reply_text("无权访问该记录")
        return
    ...
    f0_sign = "+" if f0_diff >= 0 else ""
    f1_sign = "+" if f1_diff >= 0 else ""
    f2_sign = "+" if f2_diff >= 0 else ""
    gender_sign = "+" if gender_diff >= 0 else ""
    
    report = f"""
**对比分析** (#{analysis1.id} vs #{analysis2.id})
...
**音高对比**
├ F0 均值: {format_number(params1.get('f0_mean'))} → {format_number(params2.get('f0_mean'))} ({f0_sign}{format_number(f0_diff)} Hz)
...
**共振峰对比**
├ F1: {format_number(params1.get('f1_mean'))} → {format_number(params2.get('f1_mean'))} ({f1_sign}{format_number(f1_diff)} Hz)
├ F2: {format_number(params1.get('f2_mean'))} → {format_number(params2.get('f2_mean'))} ({f2_sign}{format_number(f2_diff)} Hz)
└ F3: {format_number(params1.get('f3_mean'))} → {format_number(params2.get('f3_mean'))}

**性别评分对比**
├ 综合评分: {int(params1.get('gender_score', 0))} → {int(params2.get('gender_score', 0))} ({gender_sign}{int(gender_diff)} 分)
